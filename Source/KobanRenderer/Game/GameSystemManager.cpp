...
void GameSystemManager::Execute() {
	...
	// 三角形描画
	{
		try {
			...
			//テクスチャをピクセルシェーダーにセット
			D3D.mDeviceContext->PSSetShaderResources(0, 1, mTexture.mSrv.GetAddressOf());

			//デバイスコンテキストへの描画命令
			D3D.mDeviceContext->Draw(4, 0);
		}
		catch (_com_error& e) {
			printf("GameSystemManagerでエラー");
		}
